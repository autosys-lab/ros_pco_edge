...
int main(int argc, char* argv[])
{
    ...
    for(int i=argc;i>1;)
    {
        ...
        if((a=strstr(argv[i],"-l")))
        {
            ...
            mylog=new CPco_Log("pco_camera_grab.log");
            if(x>0)
            {
                x|=0x03;
                mylog->set_logbits(x);
    ...
    if(help)
    {
        printf("usage: %s options\n"
               ...
               "-l[0...]  actual loglevel default no logging enabled\n"
               "-h,-?,? this message\n",argv[0]);
        exit(0);
    }
...
/*
//adapt transfer_size for pixelfly usb with coding
//more packages, but better overall response
  if(camera->getCoding())
  {
   int packet_size=grabber->Get_Async_Packet_Size();
   if(aps>64*1024)
    grabber->Set_Async_Packet_Size(packet_size);
    printf("grabber->Packet_Size set to %d\n",packet_size);
  }
*/
    ...
    while(c!='x')
    {
        ...
        else if(c=='b')
        {
            ...
            if(description.wBinHorzSteppingDESC==1) //linear
            {
                ...
                {
                    new_binhorz=binhorz;
                    printf("wrong input (linear description.wMaxBinHorzDESC=%d) use horizontal binning %d\n",description.wMaxBinHorzDESC,new_binhorz);
                }
            }
            else //binary 0 or 2
            {
                ...
                {
                    new_binhorz=binhorz;
                    printf("wrong input (binary description.wMaxBinHorzDESC=%d) use horizontal binning %d\n",description.wMaxBinHorzDESC,new_binhorz);
                }
            }
            ...
            if(description.wBinVertSteppingDESC==1) //linear
            {
                ...
                {
                    new_binvert=binvert;
                    printf("wrong input (linear description.wMaxBinVertDESC=%d) use vertical binning %d\n",description.wMaxBinVertDESC,new_binvert);
                }
            }
            else //binary 0 or 2
            {
                ...
                {
                    new_binvert=binvert;
                    printf("wrong input (binary description.wMaxBinVertDESC=%d) use vertical binning %d\n",description.wMaxBinVertDESC,new_binvert);
                }
            }
        ...
        {
            printf("\n");
            printf("call grab_single\n");
            err=grab_single(grabber,NULL);
            printf("call grab_single returned 0x%x\n",err);

            err=get_image(grabber,NULL,1,1);
            printf("call get_image(1,1) returned 0x%x\n",err);

            printf("call grab_single_async\n");
            err=grab_single_async(grabber,NULL);
            printf("call grab_single_async returned 0x%x\n",err);
        ...
        else if(c=='8')
        {
            ...
            int p;
            printf("\n");
            printf("enter new Packet size actual %d KB ...<CR>: ",packet_size/1024);
            get_number(number,5);
            p=atoi(number);
            if(p>4096)
                p=4096;
            if(p<1)
                p=1;
            printf("Set new Packet size %d\n",p*1024);
            grabber->Set_Async_Packet_Size(p*1024);
        }
    }
