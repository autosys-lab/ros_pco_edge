...
#include <pco_driver/pco_classes/Cpco_com_usb.h>
...
DWORD CPco_com_usb::usb_get_endpoints()
{
    ...
    case USB_PID_CAM_PANDA_30:      // 0x000C  Panda FX3 USB3.0 Interface
    ...
    case USB_PID_CAM_FLOW_20:         // 0x0007  AVR32 flow
    case USB_PID_CAM_EDGEHS_20:       // 0x0008  AVR32 edge clhs
    case USB_PID_CAM_PANDA_20:        // 0x000B  AVR32 Panda 
    case USB_PID_DP3_MAIN:            // 0x000D  AVR32 DP3 Debug and Prog port
    case USB_PID_CAM_EDGE_42BI_PRG:   // 0x000E  edge 4.2 bi internal USB 2.0 Interface
    case USB_PID_CAM_PANDA_42BI_PRG:  // 0x0010  panda 4.2 bi internal USB 2.0 Interface
    case USB_PID_CAM_EDGE_260_PRG:    // 0x0012  edge 26 internal USB 2.0 Interface
...
DWORD CPco_com_usb::Open_Cam_Ext(DWORD num,SC2_OpenStruct *open ATTRIBUTE_UNUSED)
{
    ...
    writelog(INIT_M, (PCO_HANDLE)1,"Open_Cam_Ext: Common Version %02d.%02d.%02d",COMMON_VERSION,COMMON_SUBVERSION,COMMON_SUBAGE);
    writelog(INIT_M, (PCO_HANDLE)1,"Open_Cam_Ext: USB Version %02d.%02d.%02d",VERSION,SUBVERSION,SUBAGE);


    if(sem_init(&sMutex, 0, 1) == -1)
     writelog(ERROR_M, (PCO_HANDLE)1, "Could not get semaphore sMutex.");
    else
     writelog(INTERNAL_4_M, (PCO_HANDLE)1, "semaphore sMutex 0x%x created");
    ...
    if(  (desc.idProduct==USB_PID_CAM_FLOW_20)||(desc.idProduct==USB_PID_CAM_EDGEHS_20)||(desc.idProduct==USB_PID_CAM_PANDA_20)
       ||(desc.idProduct==USB_PID_DP3_MAIN)||(desc.idProduct==USB_PID_CAM_EDGE_42BI_PRG)||(desc.idProduct==USB_PID_CAM_PANDA_42BI_PRG)
       ||(desc.idProduct==USB_PID_CAM_EDGE_260_PRG))
    {
        writelog(INIT_M,hdriver,"Open_Cam_Ext: FLOW_20 or EDGEHS_20 or _PRG use alt_set 1");
        alt_set=1;
    }
...
//set interface output format to coding
    if(desc.idProduct==USB_PID_CAM_PIXFLY_20)
    {
//first set 14Bit Mode
     WORD wFormat = 0;
     err=PCO_SetInterfaceOutputFormat(SET_INTERFACE_USB,wFormat);
     if(err==PCO_NOERROR)
     {
//if successful set coding
      wFormat=IMG_TRANSFER_MODE_BITSTUFFING;
      err=PCO_SetInterfaceOutputFormat(SET_INTERFACE_USB,wFormat);

      err=PCO_GetInterfaceOutputFormat(SET_INTERFACE_USB,&wFormat);
      writelog(INIT_M,hdriver,"Open_Cam_Ext: PCO_GetInterfaceOutputFormat returned 0x%x format 0x%x",err,wFormat);
      if(wFormat != IMG_TRANSFER_MODE_BITSTUFFING)
      {
...
DWORD CPco_com_usb::Close_Cam()
{
    ...
    //delete semaphor
    writelog(INTERNAL_4_M, hdriver, "Close_Cam: Close Mutex 0x%x", sMutex);
    sem_close(&sMutex);
    sem_destroy(&sMutex);
...
//@@@mbl for reboot command
    ...
    {
     usb_clear_input();
     usbcom_out(com_out,size_out,PCO_NOERROR); 
     return PCO_NOERROR;
    }
    ...
    {
     usb_in=PCO_SC2_DEF_BLOCK_SIZE; //sizeof(WORD)*2;
     buf=buffer;
     err=usb_read(buf,&usb_in,tab_timeout.command*2);
     if(err!=PCO_NOERROR)
     {
      usbcom_out(com_out,size_out,err);
      return err;
     }
     writelog(INTERNAL_1_M,hdriver,"Control_Command: usb_in %d",usb_in);
         
    
//when an edge camera is connected returned size is always a multiple of 16 and therefore can
//not be used to get the size of the telegram
//correct size must be readout from the telegram itself 
     ...
     if((com_out&0xFF3F)==IMAGE_TRANSFER_DONE_MSG)
     {
      ...
      siz=packet_size;
//checksum is uncorrect on some cameras, therefore no error return here 
      err = test_checksum(buf,(int*)&siz);
      if(err!=PCO_NOERROR)
      {
       writelog(PROCESS_M,hdriver,"IMAGE_TRANSFER_DONE Checksum error");
      }   
      ...
      if(siz==sizeof(resp))
      {
       ...
       writelog(PROCESS_M,hdriver,"Control_Command: end IMAGE_TRANSFER_DONE DataCount %d",transfer_done);
      }
      else if(siz==sizeof(resp1))
      {
       memcpy(&resp1,buf,sizeof(resp1));
       transfer_done=1;
       writelog(PROCESS_M,hdriver,"Control_Command: end IMAGE_TRANSFER_DONE no DataCount");
      }
...
//Wait for next response
    ...
    if(packet_size > sizeof(SC2_Failure_Response))
     size=packet_size;
    else
...
DWORD CPco_com_usb::transfer_msgwait()
{
    DWORD err=PCO_NOERROR;
    unsigned char buffer[PCO_SC2_DEF_BLOCK_SIZE];
    unsigned char *buf;
    ...
    if(transfer_done>0)
    {
     writelog(PROCESS_M,hdriver,"transfer_msgwait: transfer_done %d return",transfer_done);
    ...
    {
     if (errno == ETIMEDOUT)
      writelog(ERROR_M,hdriver,"mutex timed out");
     else
      writelog(ERROR_M,hdriver,"some other error");
    }
    ...
    usb_in=20; //PCO_SC2_DEF_BLOCK_SIZE; //sizeof(WORD)*2;
    buf=buffer;
    err=usb_read(buf,&usb_in,10);
    if(err!=PCO_NOERROR)
    {
     sem_post(&sMutex);
     return 0;
    ...
    writelog(PROCESS_M,hdriver,"transfer_msgwait: usb_in %d packet_size %d com_out 0x%04x",usb_in,packet_size,com_out);
...
//checksum is uncorrect on some cameras, therefore no error return here 
        ...
        if(err!=PCO_NOERROR)
        {
         writelog(PROCESS_M,hdriver,"transfer_msgwait: Checksum error");
        }   
        ...
        if(siz==sizeof(resp))
        {
         memcpy(&resp,buf,sizeof(resp));
         transfer_done=resp.dwImageDataCount;
         writelog(PROCESS_M,hdriver,"transfer_msgwait: IMAGE_TRANSFER_DONE DataCount %d",transfer_done);
        }
        else if(siz==sizeof(resp1))
        {
         memcpy(&resp1,buf,sizeof(resp1));
         transfer_done=1;
         writelog(PROCESS_M,hdriver,"transfer_msgwait: end IMAGE_TRANSFER_DONE no DataCount");
        }
        else if((com_out|RESPONSE_OK_CODE)==FATAL_ERROR_MSG)
        {
         writelog(ERROR_M,hdriver,"transfer_msgwait: FATAL_ERROR",siz);
         sem_post(&sMutex);
         return 0;
        }
